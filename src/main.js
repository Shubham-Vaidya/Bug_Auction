/* =====================================================
   ...
   Landing Page + Admin Dashboard + Team Mode
   ===================================================== */

// ── App State ──────────────────────────────────────────
const state = {
  page: 'landing',       // landing | admin-login | admin-dashboard | team-join | team-auction | leaderboard
  roomId: null,
  teams: [],
  currentBug: null,
  bugOwner: null,
    ...
    <div class="screen-center">
      <div class="card landing-card border-green pulse-green slide-up">
        ...
        <div class="landing-btn-group">
          <button class="btn btn-green btn-lg btn-full" onclick="go('admin-login')">
            ⚡ ENTER AS ADMIN
          </button>
          <button class="btn btn-purple btn-lg btn-full" onclick="go('team-join')">
            👥 ENTER AS TEAM
          </button>
        </div>
  ...
  return `
    <div class="screen-center">
      <div class="card auth-card border-green pulse-green slide-up">
        <div class="text-xs text-sec mb-8" style="letter-spacing:3px;cursor:pointer" onclick="go('landing')">← BACK TO PORTAL</div>
        ...
        <button class="btn btn-green btn-full" onclick="adminLogin()">INITIALIZE SESSION ⚡</button>
        <p class="mt-20 text-xs text-sec text-center cursor-pointer" onclick="go('team-join')">
  ...
  return `
    ...
    <div class="top-bar">
      ...
      <div class="btn-row">
        ...
        <button class="btn btn-purple btn-sm" onclick="go('landing')">LOGOUT</button>
      </div>
    </div>
      ...
      <div class="admin-grid">
        ...
        <div class="panel">
          <div class="card border-green">
            ...
            <button class="btn btn-green btn-full mb-16" onclick="createRoom()">
              ＋ CREATE NEW ROOM
            </button>
        ...
        <div class="panel">
          <div class="card">
            ...
            <div class="flex gap-8" style="flex-wrap:wrap">
              ${bugPool.map((b, i) => `
                <button class="btn btn-sm btn-blue" onclick="adminSetBug(${i})" title="${b.name}">
                  ${b.id}
                </button>`).join('')}
            </div>
          </div>
        ...
        <div class="panel">
          ...
          <div class="card">
            ...
            <div style="display:flex;flex-direction:column;gap:12px">
              <button class="btn btn-blue btn-full" onclick="go('leaderboard')">📊 View Leaderboard</button>
    ...
    <div id="assign-modal" class="modal-overlay">
      <div class="modal-box">
        ...
        <div class="btn-row mt-24">
          <button class="btn btn-green flex-1" onclick="confirmAssign()">CONFIRM</button>
          <button class="btn btn-purple flex-1" onclick="closeModal()">CANCEL</button>
        </div>
      </div>
    </div>
  `;
    ...
    <div class="screen-center">
      <div class="card join-card border-purple pulse-purple slide-up">
        ...
        <div style="margin-top:32px">
          ...
          <!-- Language Chips — replaces dropdown -->
          <div style="margin-top:24px;margin-bottom:24px">
            <span class="lang-chips-label">Language Environment</span>
            <div class="lang-chips-group" id="lang-chip-group">
              <div class="lang-chip ${state.selectedLang === 'JavaScript' ? 'selected' : ''}" data-lang="JavaScript" onclick="selectLang('JavaScript')">
                <span class="lang-chip-icon">🟨</span> JavaScript
              </div>
              <div class="lang-chip ${state.selectedLang === 'Python' ? 'selected' : ''}" data-lang="Python" onclick="selectLang('Python')">
                <span class="lang-chip-icon">🐍</span> Python
              </div>
              <div class="lang-chip ${state.selectedLang === 'C++' ? 'selected' : ''}" data-lang="C++" onclick="selectLang('C++')">
                <span class="lang-chip-icon">⚙️</span> C++
              </div>
              <div class="lang-chip ${state.selectedLang === 'Java' ? 'selected' : ''}" data-lang="Java" onclick="selectLang('Java')">
        ...
        <button class="btn btn-purple btn-full" onclick="teamJoin()">ESTABLISH CONNECTION →</button>
        <p class="mt-20 text-xs text-sec text-center cursor-pointer" onclick="go('admin-login')">
...
window.teamJoin = () => {
  ...
  // selectedLang is already stored in state by selectLang()
  go('team-auction');
};

// ── Language chip selection handler ────────────────────
window.selectLang = (lang) => {
  state.selectedLang = lang;

  // Use data-lang attribute — reliable, not fragile textContent matching
  const group = document.getElementById('lang-chip-group');
  if (!group) return;

  group.querySelectorAll('.lang-chip').forEach(chip => {
  ...
  return `
    ...
    <div class="top-bar">
      ...
      <div class="btn-row">
        ...
        <button class="btn btn-purple btn-sm" onclick="go('landing')">DISCONNECT</button>
      </div>
    </div>
...
function LeaderboardPage() {
  ...
  return `
    <div class="top-bar">
      ...
      <button class="btn btn-purple btn-sm" onclick="history.back ? history.back() : go('landing')">← BACK</button>
    </div>
    ...
    <div class="page">
      ...
      <div class="btn-row btn-row-center mt-40">
        <button class="btn btn-green" onclick="go('landing')">← BACK TO PORTAL</button>
      </div>
    </div>
  `;
}
